...
export const MachineCard = ({ machines } : MachineCardProps) => {
  return (
    <div className="flex flex-col h-auto mx-auto max-w-4xl bg-delawareRed dark:bg-lightestNavy rounded-2xl">
      <Accordion type="single" collapsible className="bg-delawareRed dark:bg-lightestNavy w-full rounded-xl p-[10px]">
        ...
        <AccordionItem value={`item-${index}`} key={machine.machineCode} className="min-h-[50px] border-0">
          <AccordionTrigger className="flex items-center w-full h-full text-white hover:no-underline hover:bg-[#d13a32] dark:hover:bg-[#5C658C] rounded-xl px-[10px]">
            <div className={`${machine.status == "Running" ? "bg-[#3CEF3C]":"bg-rancidRed dark:bg-delawareRed"} w-5 h-5 rounded-[50%] mx-2 border-2 border-white`}></div>
            <p className="">
