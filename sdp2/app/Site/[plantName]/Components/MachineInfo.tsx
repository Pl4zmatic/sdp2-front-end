...
import MaintenanceDashboard from "./MaintenanceDash";
...
const MachineInfo = ({ idTechnician, idMachine }: MachineInfoProps) => {
  ...
  const lastMaintenance = new Date(machineData?.laatste_onderhoud_datum)
    .toISOString()
    .slice(0, 10);
  const nextMaintenance = new Date(machineData?.datum_toekomstige_onderhoud)
    .toISOString()
    .slice(0, 10);
  ...
  const handleSendNotification = async () => {
    ...
    console.log(
      `The machine with code ${machineData.CODE} has been stopped due to the following reasons: ${reasonsString}`,
    );
  ...
  const displayAndSortFiveMaintenances = useMemo(() => {
    ...
    return [...maintenancesPerMachine]
      .filter(
        (maintenance: Maintenance) =>
          new Date(maintenance.STARTDATE) > todayMinusThirtyDays,
      )
      .slice(0, 5);
    ...
    <div className="flex flex-col py-[10px] ml-2 mt-2">
      <div className="flex border-l-4 justify-center items-center">
        <div className="flex-col flex-1 px-[20px]">
          ...
          <div>
            <span className="font-bold">Last maintenance</span>:{" "}
            {lastMaintenance}
          </div>
          <div>
            <span className="font-bold">Next maintenance</span>:{" "}
            {nextMaintenance}
          </div>
        </div>
        <div className="flex flex-col justify-center items-end p-5">
          <div>
            {isRunning ? (
                ...
                <AlertDialogContent className="bg-white dark:bg-navy border-0">
                  <AlertDialogHeader>
                    <AlertDialogTitle className="text-darkGray dark:text-white">
                      Why do you want to{" "}
                      <span className="text-delawareRed font-semibold">
                        shut down
                      </span>{" "}
                      this machine?
                    </AlertDialogTitle>
                    <div className="flex flex-col text-darkGray dark:text-[#ADB3CC]">
                      <div className="flex items-center space-x-2 py-1">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setMaintenanceChecked(checked === true)
                          }
                          id="maintenance"
                      ...
                      <div className="flex items-center space-x-2 py-1">
                        <Checkbox
                          checked={overheatingChecked}
                          onCheckedChange={(checked) =>
                            setOverheatingChecked(checked === true)
                          }
                          id="overheating"
                          className="w-[20px] h-[20px] border-gray-400 dark:border-[#ADB3CC] data-[state=checked]:bg-delawareRed data-[state=checked]:border-delawareRed"
                        />
                      ...
                      <div className="flex items-center space-x-2 py-1">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            setInspectionChecked(checked === true)
                          }
                          id="inspection"
                          className="w-[20px] h-[20px] border-gray-400 dark:border-[#ADB3CC] data-[state=checked]:bg-delawareRed data-[state=checked]:border-delawareRed"
                        />
                      ...
                      {otherChecked && (
                        <Textarea
                          className="mt-2 bg-white dark:bg-navy text-gray-700 dark:text-white"
                  ...
                  <AlertDialogFooter>
                    <AlertDialogCancel
                      onClick={() => handleUncheckingCheckboxes()}
                      className="bg-delawareRed text-white border-0 hover:bg-[#F16776] hover:text-white"
                    >
                      Cancel
                    </AlertDialogCancel>
                    <AlertDialogAction
                      className="bg-white dark:bg-lightestNavy text-gray-700 dark:text-white border border-gray-300 dark:border-0 hover:bg-gray-100 dark:hover:bg-[#5C658C]"
                      onClick={() => {
              ...
              <Play
                className="w-6 h-6 border-5 cursor-pointer"
                onClick={() => updateMachineStatus("running")}
              />
            )}
          </div>
        </div>
          ...
          <DialogContent className="w-auto max-w-[75%] min-w-[50%]">
            <DialogHeader>
              <DialogTitle className="text-center">
                Maintenances from the last 30 days
              </DialogTitle>
            </DialogHeader>
            <MaintenanceDashboard
              maintenances={displayAndSortFiveMaintenances}
            />
          </DialogContent>
        </Dialog>
      </div>
