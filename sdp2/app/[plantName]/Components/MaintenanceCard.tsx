...
const MaintenanceCard = ({ maintenances }: MaintenanceCardProps) => {
  ...
  async function downloadPdfFile(maintenance: Maintenance) {
    ...
    const lines = [
      ...
      `Current State: ${
        maintenance.CURRENTSTATESTRING === "ProgressState" ? "In progress" : maintenance.CURRENTSTATESTRING === "FinishedState" ? "Finished" : "Planned"
      }`
    ];
          ...
          {maintenances.length === 0 ? (
            ...
            <div className="flex w-full gap-5 justify-center items-center flex-wrap">
              {displayAndSortFiveMaintenances.map((maintenance) => (
                    ...
                    <Card key={maintenance.MAINTENANCEID} className="min-h-[50px] h-auto border-0 w-[40%] rounded-xl p-[2%] bg-neutral-100 dark:bg-lightestNavy flex items-center justify-center">
                      <CardContent className="flex flex-col items-center justify-center h-full w-full text-black p-0 gap-5 text-wrap">
                          <div className="flex items-center justify-center">
                            {maintenance.CURRENTSTATESTRING.toLowerCase() === "finishedstate" ? (
                              <Check className="text-black dark:text-white" />
                            ) : maintenance.CURRENTSTATESTRING.toLowerCase() === "progressstate" ? (
                        ...
                        <div className="flex item-center justify-center gap-5">
                          <div className="flex flex-col w-full justify-center items-center gap-5 text-black dark:text-white text-wrap mr-auto">
                            <div className="flex flex-col h-full w-full gap-3 text-wrap">
                              <div><span className="font-semibold text-lg">Machine: </span>{maintenance.MACHINECODE}</div>
                              <div><span className="font-semibold text-lg">Date: </span>{`${new Date(maintenance.STARTDATE).getFullYear()}-${new Date(maintenance.STARTDATE).getMonth() + 1 < 10 ? "0":""}${new Date(maintenance.STARTDATE).getMonth() + 1}-${new Date(maintenance.STARTDATE).getDate()}`}</div>
                              <div className="text-wrap"><span className="font-semibold text-lg">Reason: </span>{maintenance.REASON}</div>
                              <div><span className="font-semibold text-lg">Remarks: </span>{maintenance.REMARKS}</div>
                            </div>
                          </div>
                        </div>
